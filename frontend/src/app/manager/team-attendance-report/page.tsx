...
export default function TeamAttendanceReportPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [mgr, team] = await Promise.all([
          fetchCurrentManager(),
          fetchTeamAttendance(ymd(monthStart), ymd(monthEnd)),
        ]);
        if (cancelled) return;
        setManager(mgr);
        setData(team);
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        >
          <nav className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium text-gray-800">Team Attendance Report</span>
          </nav>
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          <TeamAttendanceReport data={data} loading={loading} monthLabel={monthLabel} />
        </main>
      </div>
    </div>
  );
